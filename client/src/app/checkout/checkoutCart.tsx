...
export default function CheckoutCart() {
  ...
  return (
    <>
      <Box h={'60%'} overflow={'auto'} w={!smallerThan740 ? '50%' : '100%'} mt={'2%'}>
        <Box w={'98%'} h={'50px'} pl={'40px'} display={'flex'} justifyContent={'space-between'} alignItems={'center'} flexDir={'row'}>
          <Text fontWeight={'semibold'}>PRODUCT NAME</Text>
          <Text fontWeight={'semibold'} w={'100px'}>SUBTOTAL</Text>
        </Box>
        {
          cart?.map(product => {
            return(
              <Box key={product.idCartEntry}>
                ...
                <Box display={'flex'} flexDir={!smallerThan740 ? 'row' : 'column'} w={'98%'} justifyContent={'space-between'}>
                  <ProductCardCart product={product}/>
                  {
                    ...
                    <Box
                      ...
                      >
                      <Text h={"30px"}fontWeight={"semibold"}>${product.SalePrice * product.Quantity}</Text>
                    </Box>
                  }
              </Box>
            </Box>
