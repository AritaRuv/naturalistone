...
const ProjecteInfo = ({params}) => {
  ...
  const { project } = useAppSelector((state: { projectsReducer: ProjectsState }) => state.projectsReducer);
  
  const dispatch = useAppDispatch();

  const { CustomerID, ProjectName, Shipping_Address, Shipping_City, Shipping_State, Shipping_ZipCode } = project;
        ...
        <VStack pl={"2vw"} pt={"2vh"} alignItems={"flex-start"}>
          ...
          <VStack pl={"1vw"} alignItems={"flex-start"}>
            <Box>
              ...
              <Text fontSize={"0.9rem"} fontWeight={"thin"}>{Shipping_Address ? Shipping_Address.toUpperCase() : "-"}</Text>
            </Box>
            <Box>
              <Text fontSize={"0.6rem"} fontWeight={"semibold"}>CITY</Text>
              <Text fontSize={"0.9rem"} fontWeight={"thin"}>{Shipping_City ? Shipping_City.toUpperCase() : "-" }</Text>
            </Box>
            <Box>
              <Text fontSize={"0.6rem"} fontWeight={"semibold"}>ZIP CODE</Text>
              <Text fontSize={"0.9rem"} fontWeight={"thin"}>{Shipping_ZipCode ? Shipping_ZipCode.toUpperCase() : "-"}</Text>
            </Box>
            <Box>
              <Text fontSize={"0.6rem"} fontWeight={"semibold"}>STATE</Text>
              <Text fontSize={"0.9rem"} fontWeight={"thin"}>{Shipping_State ? Shipping_State.toUpperCase() : "-"}</Text>
            </Box>
          </VStack>
