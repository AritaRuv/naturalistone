...
const UserButtonsContainer:React.FC<IShowMenu> = ({site}) => {
            ...
            return(
              <Box key={i}>
                <Link href={site === "navbar" ? "/profile" : "" }>
                  <Button
                    borderLeft={site=== "navbar" ? ("unset"):(appContext?.showMenu === button.name ? "3px solid black" : "unset")}
                    _hover={{
                      fontWeight: "semibold"
                    }} 
                    rounded={0} h={"4vh"} 
                    ...
                    fontWeight={"normal"} 
                    name={button.name}
                    textTransform={"uppercase"}>{button.name}</Button>
                </Link>
              </Box>
            );
          })
